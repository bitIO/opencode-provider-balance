...
export class DeepSeekProvider implements BalanceProvider {
    ...
    async fetchBalance(): Promise<BalanceSnapshot> {
        ...
        try {
            ...
            if (error instanceof Error && error.name === "AbortError") {
                throw new BalanceFetchError(
                    `${this.name}: balance request timed out`,
                );
            }
            throw new BalanceFetchError(
                `${this.name}: network error fetching balance`,
            );
        }

        if (!response.ok) {
            throw new BalanceFetchError(
                `${this.name}: balance request failed (HTTP ${response.status})`,
            );
        }
        ...
        try {
            ...
            throw new BalanceFetchError(
                `${this.name}: invalid response from balance API`,
            );
        }
