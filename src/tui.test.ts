...
describe("buildCommandBindings", () => {
  ...
  test("defaults bind only the toggle to leader+shift+b", () => {
    expect(buildCommandBindings(parseOptions({}))).toEqual([DEFAULT_TOGGLE]);
  });

  test("keybind override binds the toggle to the given key", () => {
    expect(buildCommandBindings(parseOptions({ keybind: "ctrl+b" }))).toEqual([
      { ...DEFAULT_TOGGLE, key: "ctrl+b" },
    ]);
  });
  ...
  test("refreshKeybind adds a refresh binding alongside the default toggle", () => {
    expect(buildCommandBindings(parseOptions({ refreshKeybind: "ctrl+r" }))).toEqual([
      DEFAULT_TOGGLE,
      REFRESH,
    ]);
  });
  ...
  test('refreshKeybind "none" keeps only the default toggle', () => {
    expect(buildCommandBindings(parseOptions({ refreshKeybind: "none" }))).toEqual([
      DEFAULT_TOGGLE,
    ]);
  });
  ...
  test("both overrides produce both bindings", () => {
    expect(
      buildCommandBindings(parseOptions({ keybind: "ctrl+b", refreshKeybind: "ctrl+r" })),
    ).toEqual([
      { ...DEFAULT_TOGGLE, key: "ctrl+b" },
      REFRESH,
    ]);
